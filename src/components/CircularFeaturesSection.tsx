...
const CircularFeaturesSection = () => {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-background via-muted/10 to-primary/5 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-20 left-20 w-32 h-32 border-2 border-primary rounded-full animate-spin" style={{ animationDuration: '30s' }}></div>
        <div className="absolute bottom-20 right-20 w-24 h-24 border border-primary/50 rounded-full animate-pulse"></div>
      </div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className={`relative w-full mx-auto flex items-center justify-center ${
          isMobile ? 'h-[400px] max-w-sm' : 'h-[600px] max-w-4xl'
        }`}>
          {/* Center Logo */}
          <div className={`absolute z-20 rounded-full bg-white shadow-2xl flex items-center justify-center border-4 border-primary/20 ${
            isMobile ? 'w-20 h-20' : 'w-32 h-32'
          }`}>
            <img 
              src="/lovable-uploads/ab90ba4e-121b-4049-b65d-dec211ad12c3.png" 
              alt="SurakshaLMS Logo" 
              className={`object-contain ${isMobile ? 'w-12 h-12' : 'w-20 h-20'}`}
            />
          </div>

          {/* Circular Orbit Paths */}
          <div className={`absolute border-2 border-dashed border-primary/20 rounded-full ${
            isMobile ? 'w-48 h-48' : 'w-80 h-80'
          }`}></div>
          <div className={`absolute border-2 border-dashed border-primary/15 rounded-full ${
            isMobile ? 'w-80 h-80' : 'w-[600px] h-[600px]'
          }`}></div>
          ...
          <div className={`absolute animate-spin ${
            ...
            {features.map((feature, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={`bg-gradient-to-br ${feature.secondItemBGColor} rounded-full flex items-center justify-center shadow-lg transition-all duration-300 cursor-pointer group animate-spin relative ${
                      isMobile 
                        ? 'w-10 h-10 hover:shadow-lg' 
                        : 'w-20 h-20 hover:shadow-xl hover:scale-110'
                    }`}
                    style={{
                  ...
                  >
                    <div className={`transition-transform duration-300 ${
                      ...
                      {isMobile ? (
                        <div className="w-4 h-4 text-white flex items-center justify-center">
                          {React.cloneElement(feature.iconUrl, { className: 'w-4 h-4 text-white' })}
                        </div>
                      ) : (
                        feature.iconUrl
                      )}
                    ...
                    <div 
                      className={`absolute bottom-full left-1/2 -translate-x-1/2 text-primary font-medium whitespace-nowrap z-30 pointer-events-none ${
                        isMobile ? 'mb-1 text-[8px]' : 'mb-2 text-xs'
                      }`}
                    ...
                    <div className={`transition-transform duration-300 ${
                      ...
                      {isMobile ? (
                        <div className="w-3 h-3 text-white flex items-center justify-center">
                          {React.cloneElement(feature.iconUrl, { className: 'w-3 h-3 text-white' })}
                        </div>
                      ) : (
                        feature.iconUrl
                      )}
        ...
        <div className="text-center mt-16">
          ...
          <button className="px-8 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary-dark transition-colors duration-300 font-medium">
